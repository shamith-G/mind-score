(() => {
  ...
  if (apiTargetEl) apiTargetEl.textContent = API_BASE.replace(/^https?:\/\//, "");

  // ---------------------------------------------------------
  // Theme defaults to dark mode only
  // ---------------------------------------------------------
  document.documentElement.setAttribute("data-theme", "dark");
  try { localStorage.setItem(THEME_KEY, "dark"); } catch (_) { /* storage unavailable */ }

  // ---------------------------------------------------------
  // Toasts (transient, non-blocking status messages)
  // ---------------------------------------------------------
  const toastHost = document.getElementById("toast-host");
  function toast(message, { variant = "default", duration = 2600 } = {}) {
  ...
  // ---------------------------------------------------------
  // Ambient signal-strip waveform (decorative, aria-hidden)
  // ---------------------------------------------------------
  function drawSignalStrip() {
    const path = document.getElementById("signal-path");
    if (!path) return;
    const width = 900, mid = 30, points = 60;
    let d = `M0,${mid}`;
    for (let i = 1; i <= points; i++) {
      const x = (width / points) * i;
      // layered sines for an organic, non-repeating trace
      const y =
        mid +
        Math.sin(i * 0.35) * 9 +
        Math.sin(i * 0.9 + 1.2) * 4 +
        (Math.sin(i * 0.12) > 0.6 ? Math.sin(i * 4) * 6 : 0);
  ...
  // ---------------------------------------------------------
  // Segmented control (stress_level) — full radiogroup semantics
  ...
  function selectSegment(btn, { focus = false } = {}) {
  ...
  segButtons.forEach((btn, idx) => {
    ...
    btn.addEventListener("keydown", (e) => {
      ...
      const next = segButtons[(idx + dir + segButtons.length) % segButtons.length];
      selectSegment(next, { focus: true });
    });
  });
  ...
  function renderError(label, copy) {
    ...
    showState("error");
    toast(label, { variant: "error", duration: 3200 });
  }
